...
**Mimari not:** global backstop + named policy stratejisi ve middleware sırası (`UseAuthentication` → `UseRateLimiter`, endpoint-specific policy için zorunlu) `docs/ARCHITECTURE.md` §7.5'e eklendi.
| P10 | [x] | P3 | **MVC N+1 HTTP çağrısı** | §13 kullanım-doğrulamalı tarama tüm MVC controller/service katmanına uygulandı. **Mevcut durum:** analytics sayfası zaten tek aggregate endpoint'e (`GetAnalyticsPageAsync` → `/dashboard/analytics/page`) geçmişti — `AnalyticsController` tek çağrıyla `AnalyticsPageDto` alıp view model'i dolduruyor; server-side `GetAnalyticsPageQueryHandler` paylaşılan lookup'ları (exercises/programs) **bir kez** yükleyip volume-trend/muscle-distribution/PR/exercise-progress alt verilerini tek yanıtta birleştiriyor. **Eksik kalan (bu maddede tamamlanan):** MVC `DashboardService`/`IDashboardService` hâlâ artık hiçbir controller/view tarafından tüketilmeyen **ölü** fan-out client metotlarını (`GetVolumeTrendAsync`, `GetExerciseProgressAsync`, `GetMuscleGroupDistributionAsync`, `GetPersonalRecordsAsync`) barındırıyordu → çözüm geneli grep ile yalnızca kendi declaration+implementation'larında geçtikleri (sıfır tüketici) doğrulandıktan sonra kaldırıldı (R6/R11 ölü-kod-bırakma prensibi). Korunanlar: `GetDashboardAsync`, `GetWeightTrendAsync`, `GetAnalyticsPageAsync`. DTO'lar (`VolumeTrendPointDto` vb.) `AnalyticsPageDto` içinde taşındığından dokunulmadı. **BİLİNÇLİ kapsam dışı:** (1) Granular API endpoint'leri (`GetVolumeTrendEndpoint`/`GetExerciseProgressEndpoint`/`GetMuscleGroupDistributionEndpoint`/`GetPersonalRecordsEndpoint`) — public REST yüzeyi olarak korundu, P10 yalnızca MVC katmanı fan-out'unu kapsar. (2) `WorkoutSessionsController.LoadProgramSplitsMapAsync` — program başına split lookup'ı gerçek bir fan-out ama zaten `Task.WhenAll` ile paralelleştirilmiş; analytics dışı olduğundan ve tek aggregate endpoint gerektireceğinden ayrı bir madde kapsamındadır. (3) `HomeController` `Task.WhenAll(dashboard, weightTrend)` — bağımsız sağlıklı paralellik, N+1 değil. **Doğrulama:** dead-method kaldırma sonrası tam çözüm build 0 hata. |
| P11 | [x] | P3 | **`DateTime.Now` vs `DateTime.UtcNow` — yerel zamanda kalma kararı** | **BİLİNÇLİ MİMARİ KARAR (won't-do): UTC'ye hiçbir zaman geçilmeyecek.** §13 proje-geneli tarama, kod tabanının **zaten %100 tutarlı** şekilde yerel zaman (`DateTime.Now`/`DateTime.Today`) kullandığını ve **hiçbir yerde `DateTime.UtcNow`/`DateTimeOffset.UtcNow` bulunmadığını** doğruladı (audit `AuditableEntityInterceptor`, `OutboxInterceptor`/`OutboxBackgroundService`, `TokenService` expiry, `RefreshTokenRepository` expiry kontrolü, tüm domain event `OccurredOn`'ları ve tüm analytics/dashboard tarih-aralığı filtreleri dahil). Uygulama tek zaman-dilimi bağlamında çalıştığından ve tüm tarih karşılaştırmaları / `GetById*` / range sorguları yerel zamanla hizalı olduğundan UTC'ye geçiş **kasıtlı olarak yapılmayacaktır**; madde won't-do olarak kapatıldı (kod değişikliği yok). **Gözlem (davranışsal etki yok, P11 kapsamı dışı):** `OccurredOnUtc`/`ProcessedOnUtc`/`ExpiresOnUtc` gibi `...Utc` son ekli alanlara yerel zaman atanıyor — bu yalnızca bir **isimlendirme tutarsızlığı** olup gelecekte opsiyonel bir adlandırma temizliğiyle (`...Utc` → `...On`) netleştirilebilir. |
